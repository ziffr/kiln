import { generateWorkflows } from "@kiln/skills";
...
export default async function handler(req: Req, res: Res): Promise<void> {
  ...
  if (!client) return;
  const body = readBody<{ feedback?: string; domain?: { commands?: unknown[] }; model?: string; effort?: string; promptOverride?: string }>(req);
  ...
  const result = await generateWorkflows(body.domain as never, provider, body.feedback);
