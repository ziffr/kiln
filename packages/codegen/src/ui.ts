...
function sidebar(struct: UiStructure, appName: string): string {
  ...
  return [
    `// Generated by @vbd/codegen ui — sidebar (sidebar-16 style); nav grouped by Business Area.`,
    ...
    `          <div className="text-xs text-muted-foreground">Generated app</div>`,
    `        </div>`,
    `      </div>`,
    `      <nav className="flex-1 overflow-y-auto">`,
    `        {navigation.map((g) => (`,
    `          <div key={g.area} className="mb-3">`,
    `            <div className="mb-1 px-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{g.area}</div>`,
    `            <div className="space-y-0.5">`,
    `              {g.items.map((i) => (`,
    `                <Link key={i.route} to={i.route} className={link(i.route)}>`,
    `                  <span className="h-1.5 w-1.5 rounded-full bg-muted-foreground/50" />{i.title}`,
    `                </Link>`,
    `              ))}`,
    `            </div>`,
    `          </div>`,
    `        ))}`,
    `        <div className="mb-3">`,
    `          <div className="mb-1 px-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">Resources</div>`,
    `          <Link to="/help" className={link("/help")}><span className="h-1.5 w-1.5 rounded-full bg-muted-foreground/50" />Help &amp; docs</Link>`,
    `        </div>`,
    `      </nav>`,
    `      <div className="flex items-center gap-2 rounded-lg p-2">`,
...
function appHeaderTsx(): string {
  return [
    `// Generated by @vbd/codegen ui — top bar: sidebar toggle + breadcrumb + search (sidebar-16 style).`,
    `import { useLocation } from "react-router-dom";`,
    `import { routeTitles, appName } from "./AppSidebar";`,
    "",
    `export function AppHeader({ onToggle }: { onToggle: () => void }) {`,
    `  const { pathname } = useLocation();`,
    `  const title = routeTitles["/" + (pathname.split("/")[1] ?? "")] ?? "";`,
    `  return (`,
    ...
    `        <input placeholder="Search…" className="h-8 w-56 rounded-md border bg-background px-3 text-sm outline-none focus:ring-1 focus:ring-ring" />`,
    `      </div>`,
    `    </header>`,
    `  );`,
    `}`,
...
function appShellTsx(): string {
  return [
    `// Generated by @vbd/codegen ui — the sidebar-16 app shell (inset content). Skin; content = the model.`,
...
function appTsx(struct: UiStructure): string {
  ...
  return [
    `// Generated by @vbd/codegen ui (shadcn) — app shell (sidebar-16) + routes (one list + detail per entity).`,
