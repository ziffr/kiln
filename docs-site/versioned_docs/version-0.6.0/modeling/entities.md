...
Date, Money, or Reference** — so the app knows what kind of information it holds. To relate one entity to
another, use **References** in the panel and pick the other entity from the list — the picker only offers
entities that exist, so a relationship can never point at a missing record.
