import { generateContexts } from "@kiln/skills";
...
export default async function handler(req: Req, res: Res): Promise<void> {
  ...
  if (!client) return;
  const body = readBody<{ feedback?: string; capabilities?: CapabilityDoc; model?: string; effort?: string; promptOverride?: string }>(req);
