...
- **User-facing docs live in `docs-site/`** (Docusaurus → GitHub Pages, versioned). **When a change
  ...
  are **English-only for now** (the Studio *app* stays bilingual EN/DE — that's `apps/web`, not the docs
  site). Purely-internal changes (refactors, tests, tooling) need no docs.
- **The docs site keeps exactly ONE version — the latest release — pinned by `lastVersion` (currently
  `0.5.1`) and served at the root; there is no version dropdown and no history (old snapshots are pruned on
  release).** The default page is the snapshot under `docs-site/versioned_docs/version-<lastVersion>/`; the
  unversioned `docs/` folder is the "Next" dev preview under `/next`. A page added/edited ONLY in `docs/`
  is invisible on the default site, so for a shipped user-facing change **edit the matching page in
  `versioned_docs/version-<lastVersion>/` too IN THE SAME COMMIT** — not just `docs/`. Sidebars are
  autogenerated; verify it renders at the default URL (no `/next` prefix), not a soft-404.
- **Docs version stays in LOCKSTEP with the app version, but docs edits DON'T have to bump the app.**
  **Gotcha:** release-please 17.x bumps a **patch for `docs:`** commits (it's not just feat/fix). So to
  correct the *current* version's docs WITHOUT a version bump, commit as **`chore(docs):`** — a genuinely
  non-releasing type (mirror-edit the `versioned_docs/version-<lastVersion>/` page + `docs/`, commit
  `chore(docs): …`). Use plain `docs:` only when you deliberately want the change to ride a patch release.
  Conversely, when a `feat`/`fix` cuts a release (any bump, incl. patch), the `docs-version` job auto-opens
  a **`chore(docs): snapshot vX.Y.Z`** PR that freezes `docs/` → the new version and bumps `lastVersion` —
  so the default docs version always equals the app version. It's `chore(docs):` on purpose: a `docs:`
  snapshot would itself bump a patch and loop. Just review & merge that PR.
...
- **LAUNCHED: `github.com/ziffr/kiln` is PUBLIC and live.** Full VBD→Kiln rename done (packages/CLI/env/IDs/prose; legacy `VBD_ANTHROPIC_API_KEY` + storage-key aliases kept for back-compat). Deployed on Vercel (auto-deploy via git integration on push to `main`); hosted demo is client-side (baked examples + all stages + code preview + full-stack export) and real-LLM `/api` works on the hosted demo — the functions are consolidated into one catch-all (`functions/router.ts` → `api/[...path].js`), under the plan cap. `main` is branch-protected (PR required + a `test-and-build` status check); the owner (`@ziffr`) has bypass rights, so direct pushes land but skip the gate.
  - **Releases are automated by release-please — DO NOT hand-roll them.** Every push to `main` runs `.github/workflows/release-please.yml`, which maintains a **release PR** (`chore(main): release X.Y.Z`) accumulating the next version + a categorized CHANGELOG from Conventional Commit messages. **To cut a release, MERGE that PR** — it bumps `package.json` + `.release-please-manifest.json`, writes `CHANGELOG.md`, and creates the `vX.Y.Z` tag + GitHub Release. Manually editing those files corrupts release-please's baseline (a hand-rolled 0.3.0 once made it skip to 0.4.0). On **every** release (minor/major AND patch) the `docs-version` job auto-opens a `chore(docs): snapshot vX.Y.Z` PR that snapshots the docs (`versioned_docs/version-X.Y.Z` + `versions.json`) AND bumps `lastVersion` in `docs-site/docusaurus.config.js` so the docs default version always tracks the app version — just review & merge it. It's committed as `chore(docs):` (a NON-releasing type) so the snapshot doesn't cut a release of its own (release-please 17.x bumps a patch for `docs:`, which would loop). Use `chore(docs):` for any docs-only fix you want to keep out of the release cycle. Verify actual repo state with `gh repo view` / `gh pr list` before touching version files.
