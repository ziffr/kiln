...
import { attributeSpecs, type AttrType, type CapabilityDoc, type DomainDoc, type ContextsDoc, type RolesDoc } from "@vbd/compiler";
 ...
 */
export function generateWorkflows(domain: DomainDoc): string {
  const eventName = new Map((domain.events ?? []).map((e) => [e.id, e.name]));
...
export function detectGaps(caps: CapabilityDoc, domain: DomainDoc): string[] {
  ...
  if (cmds.length === 0) {
    ...
    if (policies.length === 0) {
      ...
      // Behaviour + reactions present → workflow handlers generate. Remaining gaps are the upper
      // layers not yet modelled: who may act (roles), longer processes (workflows), autonomous
      // agents, and the application/implementation blueprints.
      gaps.push(
        `${policies.length} reaction handlers generate. Not yet modelled: roles/permissions, multi-step workflows, agents, and the application/implementation blueprints — the remaining methodology layers.`,
      );
    }
  }
  return gaps;
...
export function generateAll(caps: CapabilityDoc, domain: DomainDoc, contexts?: ContextsDoc, roles?: RolesDoc): CodegenReport {
  return {
