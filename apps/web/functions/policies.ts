import { generatePolicies } from "@vbd/skills";
...
export default async function handler(req: Req, res: Res): Promise<void> {
  ...
  if (!client) return;
  const body = readBody<{ feedback?: string; domain?: { events?: unknown[]; commands?: unknown[] }; capabilities?: CapabilityDoc; model?: string; effort?: string }>(req);
