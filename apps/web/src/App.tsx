...
export default function App(): React.JSX.Element {
  ...
  const stageGen: Partial<Record<StageId, { run: () => void; busy: boolean; label: string }>> = {
    capabilities: { run: () => { labelNextSave(genSave("capabilities")); void generate(); }, busy, label: t("generateBtn") },
    areas: { run: () => { labelNextSave(genSave("areas")); void generateAreas(); }, busy: contextsBusy, label: t("genAreas") },
    entities: { run: () => { labelNextSave(genSave("entities")); void generateDomainModel(); }, busy: domainBusy, label: t("genEntities") },
    behaviour: { run: () => { labelNextSave(genSave("behaviour")); void generateBehaviour(); }, busy: behaviourBusy, label: t("genBehaviour") },
    automations: { run: () => { labelNextSave(genSave("automations")); void generatePoliciesModel(); }, busy: policiesBusy, label: t("genAutomations") },
    roles: { run: () => { labelNextSave(genSave("roles")); void generateRolesModel(); }, busy: rolesBusy, label: t("genRoles") },
    workflows: { run: () => { labelNextSave(genSave("workflows")); void generateWorkflowsModel(); }, busy: workflowsBusy, label: t("genWorkflows") },
    agents: { run: () => { labelNextSave(genSave("agents")); void generateAgentsModel(); }, busy: agentsBusy, label: t("genAgents") },
  };
