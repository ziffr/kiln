...
export function ReviewPanel({ layers, critique, busy, refinable, onReview, onRefine, onSelect, t }: Props): React.JSX.Element {
  return (
    <div className="review-panel">
      <div className="review-head">✨ {t("aiReviewTitle")}</div>
      ...
      {layers.map((row) => {
        ...
        return (
          <div key={row.kind} className={`review-row ${open ? "has-findings" : ""}`}>
            <div className="review-row-head">
              ...
              <span className="review-actions">
                <button className="review-btn" onClick={() => onReview(row.kind)} disabled={isBusy}>
                  {reviewed ? t("aiReviewAgain") : t("aiReviewGo")}
                </button>
                {open && refinable(row.kind) && (
                  <button className="review-btn refine" onClick={() => onRefine(row.kind)} disabled={isBusy}>
                    {t("aiRefine")}
                  </button>
                )}
              </span>
