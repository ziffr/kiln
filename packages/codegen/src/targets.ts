/**
 * @kiln/codegen/targets — the EXECUTION-ENGINE BINDING probe (RES-002).
 *
 ...
 *
 * Pure and isomorphic (no node:*), like the rest of @kiln. Deterministic: binding in → artifacts out.
 */
...
// (`import { POSTGRES } from "@kiln/codegen"`) keep working unchanged.
...
/** SQLite column affinities (dynamic typing): booleans stored as INTEGER 0/1, dates/money as TEXT/NUMERIC. */
export const SQLITE_TYPE: Record<AttrType, string> = {
  text: "TEXT",
  number: "REAL",
  boolean: "INTEGER",
  date: "TEXT",
...
/** SQLITE adapter: the same tables as Postgres in SQLite's dialect — single file, FKs on, no RLS. */
export function sqliteAdapter(resolved: ResolvedElement[], domain: DomainDoc): string {
  ...
  const L: string[] = ["-- Generated by @kiln/codegen targets — SQLite schema. Source of truth is the model.", "PRAGMA foreign_keys = ON;", ""];
...
export function postgresAdapter(resolved: ResolvedElement[], domain: DomainDoc, roles?: RolesDoc): string {
  ...
  const L: string[] = ["-- Generated by @kiln/codegen targets (RES-002) — PostgreSQL DDL. Source of truth is the model.", ""];
  for (const id of bound) {
    const a = aggById.get(id);
    if (!a) continue;
    const table = slug(a.id);
...
export function odooAdapter(resolved: ResolvedElement[], caps: CapabilityDoc, domain: DomainDoc, roles?: RolesDoc): Record<string, string> {
  ...
  const M: string[] = ["# Generated by @kiln/codegen targets (RES-002) — Odoo models. Business logic is hand-owned (ADR-002).", "from odoo import models, fields", ""];
