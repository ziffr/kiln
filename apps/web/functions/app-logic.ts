...
export default async function handler(req: Req, res: Res): Promise<void> {
  ...
  if (!client) return;
  const body = readBody<{ capabilities?: CapabilityDoc; domain?: unknown; contexts?: unknown; feedback?: string; model?: string; effort?: string }>(req);
  ...
  const result = await generateAppLogic(body.capabilities, body.domain as never, body.contexts as never, provider, body.feedback);
