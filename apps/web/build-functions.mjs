 ...
 * load our pure-TypeScript workspace packages (`@vbd/*`, which have no build step) at runtime. We
 * esbuild each `functions/<name>.ts` into a SELF-CONTAINED `api/<name>.js` (ESM) with the `@vbd/*`
...
await build({
  ...
  format: "esm",
  target: "node20",
  // The SDK is a real npm dep available at runtime; everything else (incl. @vbd/*) is inlined.
