...
- **Docs version stays in LOCKSTEP with the app version, but docs edits DON'T bump the app.** `docs:` is a
  **non-bumping** commit type (`release-please-config.json` marks it `hidden`), so a docs-only PR merges
  with **no release and no version change** — that's how you correct the *current* version's docs
  independently (mirror-edit the `versioned_docs/version-<lastVersion>/` page + `docs/`, commit as `docs:`).
  Conversely, when a `feat`/`fix` cuts a release (any bump, incl. patch), the `docs-version` job auto-opens
  a `docs: snapshot vX.Y.Z` PR that freezes `docs/` → the new version and bumps `lastVersion` — so the
  default docs version always equals the app version. Just review & merge that PR. (Trade-off: `docs:`
  commits don't appear in the app CHANGELOG — intended, docs are decoupled from the release cycle.)
...
- **LAUNCHED: `github.com/ziffr/kiln` is PUBLIC and live.** Full VBD→Kiln rename done (packages/CLI/env/IDs/prose; legacy `VBD_ANTHROPIC_API_KEY` + storage-key aliases kept for back-compat). Deployed on Vercel (auto-deploy via git integration on push to `main`); hosted demo is client-side (baked examples + all stages + code preview + full-stack export) and real-LLM `/api` works on the hosted demo — the functions are consolidated into one catch-all (`functions/router.ts` → `api/[...path].js`), under the plan cap. `main` is branch-protected (PR required + a `test-and-build` status check); the owner (`@ziffr`) has bypass rights, so direct pushes land but skip the gate.
  - **Releases are automated by release-please — DO NOT hand-roll them.** Every push to `main` runs `.github/workflows/release-please.yml`, which maintains a **release PR** (`chore(main): release X.Y.Z`) accumulating the next version + a categorized CHANGELOG from Conventional Commit messages. **To cut a release, MERGE that PR** — it bumps `package.json` + `.release-please-manifest.json`, writes `CHANGELOG.md`, and creates the `vX.Y.Z` tag + GitHub Release. Manually editing those files corrupts release-please's baseline (a hand-rolled 0.3.0 once made it skip to 0.4.0). On **every** release (minor/major AND patch) the `docs-version` job auto-opens a `docs: snapshot vX.Y.Z` PR that snapshots the docs (`versioned_docs/version-X.Y.Z` + `versions.json`) AND bumps `lastVersion` in `docs-site/docusaurus.config.js` so the docs default version always tracks the app version — just review & merge it. `docs:` is a non-bumping commit type, so the snapshot PR (and any docs-only fix) does NOT cut a release of its own — docs move with the app on real releases but are independently editable otherwise. Verify actual repo state with `gh repo view` / `gh pr list` before touching version files.
